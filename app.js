...
// next takes the found/created user as parameter
    ...
    handlePlay = function (req, res) {
      ...
      handleLogin(req, res, game_id, function (user) {
        logger.debug("handleLogin calling back with: " + user);
    ...
    var getPlayerProfile = function (cas_handle, game_id, callback) {
      ...
      if (LOBBY_MODE === "amqp") {
         ...
         */
        profileResponse = notification_service.getPlayerProfile(cas_handle, game_id);
        callback(null, profileResponse);
        return;
      }
      ...
      http_request(opts, function (error, response, body) {
         ...
         */
        profileResponse = JSON.parse(body);
        if (profileResponse.exception) {
          callback(profileResponse.exception, null);
        } else {
          callback(null, profileResponse);
        }
...
//
    ...
    logger.info("DB Connected: " + MONGO_URL );
